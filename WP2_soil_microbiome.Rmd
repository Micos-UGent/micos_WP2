...
selected_fields <- sample(unique(sample_data(psB_filtered5000)$Field), 40)
...
p <- plot_ordination(ps_subset_rel, pcoa.ord, color = "primers") +
  geom_point(size = 2) +
  # stat_ellipse(aes(group = Field), alpha = 0.4) +
  geom_text_repel(aes(label = gsub("MC24_", "", Field)), size = 2) +
  theme(
    axis.text = element_text(size = 10),
    axis.title = element_text(size = 12)
  )
...
```
## PcoA
Deze stap duur laaang
```{r echo = FALSE}

# Calculate ordination
pcoa.ord <- ordinate(psB_filtered5000_rel, method = "PCoA", distance = "bray")
...
#Eén van de zaken die ik doe als ik figuren maak voor een artikel is mijn lay-out vastleggen in alsvolgt:
...
#sommige categoriën zijn te klein, dan werkt deze methode niet. 
