---
title: "Macroplastic_datavisualisation_"
...
path_MAP <- file.path(path, "MaP_Hectare_woOutliers.csv")

## Check if paths exists
file.exists(path_MAP)

# Load data
MAP <- read.csv(path_MAP, sep = ";")
head(MAP)
```
...
# Create full pathways to necessary data files
path_Mean <- file.path(path, "Mean_staafdiagrammen_woOutliers.csv")

## Check if paths exists
file.exists(path_Mean)

# Load data
MAP_mean_categories <- read.csv(path_Mean, sep = ";")
head(MAP_mean_categories)
```
...
On 7 fields, no plastic was found but other macromaterials were found:
...
MAP_all <- MAP %>%
  ...
  # Data manipulation
  mutate(log_Total_litter = log10(if_else(Total_macromaterials == 0, Total_macromaterials + 1, Total_macromaterials)), #Log10 of total_macromaterials, but if total_macromaterials is zero add +1 because log10(1) = 0.
         log_Total_plastic = log10(if_else(Total_plastics == 0, Total_plastics + 1, Total_plastics)), #Log10 of total_plastics, but if total_plastics is zero add +1 because log10(1) = 0.
         log_Total_metal = log10(if_else(Total_metal == 0, Total_metal + 1, Total_metal)),
         log_Total_rubber = log10(if_else(Total_rubber == 0, Total_rubber + 1, Total_rubber)),
         log_Total_glass = log10(if_else(Total_glass == 0, Total_glass + 1, Total_glass)),
         log_Total_NP = log10(if_else(Total_NP == 0, Total_NP + 1, Total_NP)),
         log_Total_mis = log10(if_else(Total_mis == 0, Total_mis + 1, Total_mis)),
         log_Total_ceramics = log10(if_else(Total_ceramics == 0, Total_ceramics + 1, Total_ceramics)),
         log_A2 = log10(if_else(A2 == 0, A2 + 1, A2)),
         log_A17 = log10(if_else(A17 == 0, A17 + 1, A17)),
         log_A21 = log10(if_else(A21 == 0, A21 + 1, A21)),
         log_A20 = log10(if_else(A20 == 0, A20 + 1, A20)),
         log_A5 = log10(if_else(A5 == 0, A5 + 1, A5)),
         log_A6 = log10(if_else(A6 == 0, A6 + 1, A6)),
         A5_A6 = A5 + A6,
         log_A5A6 = log10(if_else(A5_A6 == 0, A5_A6 + 1, A5_A6))
  )
...
MAP_Belgium_noNA <- MAP_Belgium %>%
  filter(!is.na(distance_city), 
         ...
         !is.na(min_distance_IndorW))
```


## Dataframe Vlaanderen
```{r}
MAP_flanders <- MAP_Belgium_noNA %>%
  filter(Belgium_part == "Flanders")
...
## Dataframe Wallonië
```{r}
MAP_Wallonia <- MAP_Belgium_noNA %>%
  filter(Belgium_part == "Wallonia")
...
MAP_long <- MAP_all %>%
  ...
  mutate(log_total_counts = log10(Total_macromaterials + 1),   # Calculated in excel by Benthe
        log_counts = log10(counts + 1))  # Log transformation of each unique count per plastic category)
...
MAP_plastic_long <-  MAP_long %>%
  
  filter(Total_plastics > 0,
         str_detect(waste_category, "A")) %>%

  # Adjust the factor level
  mutate(waste_category = factor(waste_category, levels = order_plastic_category),
        
        log_total_counts = log10(if_else(Total_plastics == 0, Total_plastics + 1, Total_plastics)),   # Calculated in excel by Benthe
        log_counts = log10(counts + 1))  # Log transformation of each unique count per plastic category)
```
...
theme_poster <- function () {
  theme(
    ...
    panel.grid.minor.y = element_line(colour = "darkgrey", linewidth = 0.5), 
    # the gridlines of the x-axis
    panel.grid.major.x = element_line(colour = "darkgrey", linewidth = 0.5), 
    panel.grid.minor.x = element_line(colour = "darkgrey", linewidth = 0.5), 
    # size of the titles
...
summary <- MAP %>%
  ...
  dplyr::summarize(Mean = mean(Total_macromaterials, na.rm = TRUE),
             n = n(),
             SD = sd(Total_macromaterials,na.rm = TRUE),
             SE = sd(Total_macromaterials, na.rm = TRUE)/sqrt(n),
             CI.lower = Mean - 2*SE,
             CI.upper = Mean + 2*SE) #%>%
  #formattable
...
```
```{r}
columns_to_summarize <- c("Total_macromaterials", "Total_plastics", "Total_metal", "Total_rubber", "Total_glass", "Total_mis", "Total_NP", "Total_ceramics", "A21", "A2", "A17", "A20", "A14","A6", "A5", "A19")

summary <- data.frame(
  Variable = columns_to_summarize,
  Mean = sapply(MAP_all[ , columns_to_summarize], function(x) mean(x, na.rm = TRUE)),
  SD = sapply(MAP_all[ , columns_to_summarize], function(x) sd(x, na.rm = TRUE)),
  SE = sapply(MAP_all[ , columns_to_summarize], function(x) sd(x, na.rm = TRUE) / sqrt(sum(!is.na(x)))),
  Min  = sapply(MAP_all[, columns_to_summarize], function(x) min(x, na.rm = TRUE)),
  Max  = sapply(MAP_all[, columns_to_summarize], function(x) max(x, na.rm = TRUE)), 
  Sum = sapply(MAP_all[, columns_to_summarize], function(x) sum(x, na.rm = TRUE)))

print(summary)
```
volgorde: A2 - A17 - A20 - A6 - A5 - A21 - A19
...
MAP_mean_categories$Subcategories <- factor(MAP_mean_categories$Subcategories, levels = ordered_levels)

MAP_mean_categories <- MAP_mean_categories %>%
  mutate(
...
```{r}
Staaf_plastic <- ggplot(MAP_mean_categories[1:21, ], aes(x = Subcategories, y = mean, fill = Subcategories)) +
  ...
  theme_publish() +
  ylab("Mean number of plastics/Ha")  +
  xlab("Plastic subcategories")

Staaf_plastic
ggsave("Plots_all_litter/Staaf_plastic.png", width = 14, height = 7, dpi = 600, bg = "transparent")
```
**Metal**
```{r}
Staaf_metal <- ggplot(MAP_mean_categories[22:29, ], aes(x = Subcategories, y = mean, fill = Subcategories)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values =  fcol_metal_type, name = "Subcategories") +
  geom_errorbar(aes(ymin = mean - SE, ymax = mean + SE), width = 0.2) +
  
  theme_publish() +
  ylab("Mean number of metal/Ha")  +
  xlab("Metal subcategories")

Staaf_metal
#ggsave("Plots_all_litter/Staaf_plastic.pdf", width = 15, height = 5, dpi = 600)
```

**Rubber**

```{r}
Staaf_rubber <- ggplot(MAP_mean_categories[30:34, ], aes(x = Subcategories, y = mean, fill = Subcategories)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values =  fcol_rubber_type, name = "Subcategories") +
  geom_errorbar(aes(ymin = mean - SE, ymax = mean + SE), width = 0.2) +
  
  theme_publish() +
  ylab("Mean number of rubber/Ha")  +
...
**Glass**
```{r}
Staaf_glass <- ggplot(MAP_mean_categories[35:38, ], aes(x = Subcategories, y = mean, fill = Subcategories)) +
  geom_bar(stat = "identity") +
...
**Natural products**
```{r}
Staaf_NP <- ggplot(MAP_mean_categories[39:43, ], aes(x = Subcategories, y = mean, fill = Subcategories)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values =  fcol_natural_type, name = "Subcategories") +
  geom_errorbar(aes(ymin = mean - SE, ymax = mean + SE), width = 0.2) +
  
  theme_publish() +
  ylab("Mean number of natural products/Ha")  +
...
**Miscellaneous**
```{r}
Staaf_mis <- ggplot(MAP_mean_categories[44:46, ], aes(x = Subcategories, y = mean, fill = Subcategories)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values =  fcol_Mis_type, name = "Subcategories") +
  geom_errorbar(aes(ymin = mean - SE, ymax = mean + SE), width = 0.2) +
  
  theme_publish() +
  ylab("Mean number of miscellaneous/Ha")  +
...
**Ceramics**
```{r}
Staaf_ceramics <- ggplot(MAP_mean_categories[47:52, ], aes(x = Subcategories, y = mean, fill = Subcategories)) +
  geom_bar(stat = "identity") +
...
```
```{r}
Plot_staaf <- Staaf_plastic / (Staaf_metal+Staaf_rubber)/(Staaf_glass + Staaf_NP)/(Staaf_mis + Staaf_ceramics) +
    plot_annotation(tag_levels = "A") &
  theme(plot.margin = margin(10, 20, 10, 20))
Plot_staaf
ggsave("Plots_all_litter/Plot_staaf.png", width = 15, height = 18, dpi = 600)

```

### Histograms of all categories
#### Counts
Calculation of mean and median of Total_plastics

```{r}
mean_total_litter <- mean(MAP$Total_macromaterials)
median_total_litter <- median(MAP$Total_macromaterials)
```
**Histogram of Total_macromaterials:**

```{r}
Plot_histcounts <- ggplot(MAP) +
  geom_histogram(aes(Total_macromaterials), bins = 30, alpha = 0.7) +
...
```{r}
mean_log_total_litter <- mean(MAP_all$log_Total_litter)
median_log_total_litter <- median(MAP_all$log_Total_litter)
```

##### Histogram of Total_macromaterials log transformed:

```{r}
Plot_histlogcounts <- ggplot(MAP_all) +
  geom_histogram(aes(log_Total_litter), binwidth = 0.1) +
  geom_vline(aes(xintercept = mean_log_total_litter), linetype = "dashed", size = 1, color = '#0736ad') +
  annotate("text", x = mean_log_total_litter + 2, y = 40, label = paste("Mean =", round(mean_log_total_litter, 2)), size = 5, color = '#0736ad') +
  geom_vline(aes(xintercept = median_log_total_litter), linetype = "dashed", size = 1, color = '#07a5ad') +
  annotate("text", x = median_log_total_litter + 2, y = 50, label = paste("Median =", round(median_log_total_litter, 2)), size = 5, color = '#07a5ad') +
  ylab("Frequency") +
...
df <- data.frame(
  value = c(MAP_all$log_Total_plastic, 
            MESO_all$log_Total_mesoplastic, 
            MACRO_all$log_Total_macroplastic),
  group = factor(c(
    rep("log_Total_plastic",      nrow(MAP_all)),
    rep("log_Total_mesoplastic",  nrow(MESO_all)),
    rep("log_Total_macroplastic", nrow(MACRO_all))
  ))
)
...
Hist_plastic <- ggplot(df, aes(x = value, fill = group)) +
  geom_histogram(bins = 30, alpha = 0.8) +
  # Mean lines (dashed)
  geom_vline(data = stats, aes(xintercept = mean_val, color = group),
             linetype = "dashed", size = 1) +
  
  # Median lines (solid)
  geom_vline(data = stats, aes(xintercept = median_val, color = group),
...
**Plastic**
```{r}
Hist_plastic <- ggplot(MAP_all, aes(x = log_Total_plastic)) +
  geom_histogram(bins = 30, fill = "#cc605b", alpha = 1) +
  
  # Mean
  geom_vline(aes(xintercept = mean(log_Total_plastic)), 
             linetype = "dashed", size = 1) +
  
  annotate("text", x = mean(MAP_all$log_Total_plastic) + 1.5, y = 24, 
           label = paste("Mean =", round(mean(MAP_all$log_Total_plastic), 2)), size = 5) +
  
  # Median
  geom_vline(aes(xintercept = median(log_Total_plastic)), linetype = "dashed", size = 1, color = '#7a7878') +
  
  annotate("text", x = median(MAP_all$log_Total_plastic) + 1.6, y = 28, 
           label = paste("Median =", round(median(MAP_all$log_Total_plastic), 2)), size = 5, color = '#7a7878') +
  
  theme_publish() +
  labs(x = "Log10 of total plastic/Ha", y = "Amount of fields")

Hist_plastic
ggsave("Plots_all_litter/Hist_plastic.png", width = 7, height = 5, dpi = 600, bg = "transparent")

```

**Metal**
```{r}
Hist_metal <- ggplot(MAP_all, aes(x = log_Total_metal)) +
  geom_histogram(bins = 30, fill = "#e39b49", alpha = 0.8) +
  
  # Mean
  geom_vline(aes(xintercept = mean(log_Total_metal)), 
             linetype = "dashed", size = 1) +
  
  annotate("text", x = mean(MAP_all$log_Total_metal) + 1, y = 25, 
           label = paste("Mean =", round(mean(MAP_all$log_Total_metal), 2)), size = 5) +
  
  # Median
  geom_vline(aes(xintercept = median(log_Total_metal)), linetype = "dashed", size = 1, color = '#7a7878') +
  
  annotate("text", x = median(MAP_all$log_Total_metal) + 1, y = 50, 
           label = paste("Median =", round(median(MAP_all$log_Total_metal), 2)), size = 5, color = '#7a7878') +
...
**Rubber**
```{r}
Hist_rubber <- ggplot(MAP_all, aes(x = log_Total_rubber)) +
  geom_histogram(bins = 30, fill = "#e6db4c", alpha = 0.8) +
  
  # Mean
  geom_vline(aes(xintercept = mean(log_Total_rubber)), 
             linetype = "dashed", size = 1) +
  
  annotate("text", x = mean(MAP_all$log_Total_rubber) + 1, y = 25, 
           label = paste("Mean =", round(mean(MAP_all$log_Total_rubber), 2)), size = 5) +
  
  # Median
  geom_vline(aes(xintercept = median(log_Total_rubber)), linetype = "dashed", size = 1, color = '#7a7878') +
  
  annotate("text", x = median(MAP_all$log_Total_rubber) + 1, y = 40, 
           label = paste("Median =", round(median(MAP_all$log_Total_rubber), 2)), size = 5, color = '#7a7878') +
...
**Glass**
```{r}
Hist_glass <- ggplot(MAP_all, aes(x = log_Total_glass)) +
  geom_histogram(bins = 30, fill = "#5cd1ba", alpha = 0.8) +
  
  # Mean
  geom_vline(aes(xintercept = mean(log_Total_glass)), 
             linetype = "dashed", size = 1) +
  
  annotate("text", x = mean(MAP_all$log_Total_glass) + 1, y = 25, 
           label = paste("Mean =", round(mean(MAP_all$log_Total_glass), 2)), size = 5) +
  
  # Median
  geom_vline(aes(xintercept = median(log_Total_glass)), linetype = "dashed", size = 1, color = '#7a7878') +
  
  annotate("text", x = median(MAP_all$log_Total_glass) + 1, y = 40, 
           label = paste("Median =", round(median(MAP_all$log_Total_glass), 2)), size = 5, color = '#7a7878') +
...
**Natural products**
```{r}
Hist_NP <- ggplot(MAP_all, aes(x = log_Total_NP)) +
  geom_histogram(bins = 30, fill = "#0e8c30", alpha = 0.8) +
  
  # Mean
  geom_vline(aes(xintercept = mean(log_Total_NP)), 
             linetype = "dashed", size = 1) +
  
  annotate("text", x = mean(MAP_all$log_Total_NP) + 1, y = 25, 
           label = paste("Mean =", round(mean(MAP_all$log_Total_NP), 2)), size = 5) +
  
  # Median
  geom_vline(aes(xintercept = median(log_Total_NP)), linetype = "dashed", size = 1, color = '#7a7878') +
  
  annotate("text", x = median(MAP_all$log_Total_NP) + 1, y = 45, 
           label = paste("Median =", round(median(MAP_all$log_Total_NP), 2)), size = 5, color = '#7a7878') +
...
**Miscellaneous**
```{r}
Hist_mis <- ggplot(MAP_all, aes(x = log_Total_mis)) +
  geom_histogram(bins = 30, fill = "#6BAED6", alpha = 0.8) +
  
  # Mean
  geom_vline(aes(xintercept = mean(log_Total_mis)), 
             linetype = "dashed", size = 1) +
  
  annotate("text", x = mean(MAP_all$log_Total_mis) + 1, y = 25, 
           label = paste("Mean =", round(mean(MAP_all$log_Total_mis), 2)), size = 5) +
  
  # Median
  geom_vline(aes(xintercept = median(log_Total_mis)), linetype = "dashed", size = 1, color = '#7a7878') +
  
  annotate("text", x = median(MAP_all$log_Total_mis) + 1, y = 45, 
           label = paste("Median =", round(median(MAP_all$log_Total_mis), 2)), size = 5, color = '#7a7878') +
...
**Ceramics**
```{r}
Hist_ceramics <- ggplot(MAP_all, aes(x = log_Total_ceramics)) +
  geom_histogram(bins = 30, fill = "#7f14e3", alpha = 0.8) +
  
  # Mean
  geom_vline(aes(xintercept = mean(log_Total_ceramics)), 
             linetype = "dashed", size = 1) +
  
  annotate("text", x = mean(MAP_all$log_Total_ceramics) + 1, y = 25, 
           label = paste("Mean =", round(mean(MAP_all$log_Total_ceramics), 2)), size = 5) +
  
  # Median
  geom_vline(aes(xintercept = median(log_Total_ceramics)), linetype = "dashed", size = 1, color = '#7a7878') +
  
  annotate("text", x = median(MAP_all$log_Total_ceramics) + 1, y = 30, 
           label = paste("Median =", round(median(MAP_all$log_Total_ceramics), 2)), 
           size = 5, color = '#7a7878') +
...
```{r}
Hist_categories <- (Plot_histlogcounts+Hist_plastic) / (Hist_metal+Hist_rubber)/(Hist_glass + Hist_NP)/(Hist_mis + Hist_ceramics) +
    plot_annotation(tag_levels = "A") &
  theme(plot.margin = margin(10, 20, 10, 20))
Hist_categories
ggsave("Plots_all_litter/Hist_categories.png", width = 10, height = 15, dpi = 600)
```
...
#### Histogram stacked Macro en meso
...
**Boxplot - Country**
```{r}
Boxplot_country <- ggplot(MAP, aes(x = Country, y = Total_plastics)) +
...
cor_data
print(cor_heatmap)
ggsave("cor_heatmap_bewerkingen.pdf", plot = cor_heatmap, width = 10, height = 10)
```

### Soil index & Location index
```{r}
correlation_data <- MAP_all %>%
  dplyr::select(
    log_Total_plastic, 
    soilindex,
    location_index,
...
cor_heatmap <- ggplot(cor_data, aes(x = Var1, y = Var2, fill = value)) +
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
print(cor_heatmap)
ggsave("cor_heatmap_index.pdf", plot = cor_heatmap, width = 10, height = 10)
```
All indexes are positively correlated with log_total_plastic

### Distance only for Belgium!
```{r}
correlation_data <- MAP_Belgium %>%
  dplyr::select(
    log_Total_plastic, 
    distance_city,
    distance_highway,
...
cor_data
print(cor_heatmap)
ggsave("cor_heatmap_distance.pdf", plot = cor_heatmap, width = 10, height = 10)
```
Positive correlations between Log_total_plastics and the distances:

- Distance_city: 0.0268
- Distance_industry_Vl: 0.06002
- distance_recypark_VL: 0.060927993	
- distance_village: 0.045235305
- min_distance_waste: 0.076795499
- min_distance_industry: 0.086242689	

Negative correlations between Log_total_plastic and the distances of Belgium:

- Distance_highway: -0.0735
- distance_industrie_WL: -0.022896008	
- distance_recypark_WL: -0.029960901
- distance_waste_industriel_WL: -0.038237950
- Distance_primroutes: -0.026981026	

# Pie chart field management
```{r}
# Convert to factors (optional, for clean labeling)
Compost <- factor(MAP_all_noNA$Compost, levels = c("Yes", "No"))
Manure <- factor(MAP_all_noNA$Manure, levels = c("Yes", "No"))
Irrigation <- factor (MAP_all_noNA$Irrigation, levels = c("Other", "No", "Groundwater", "Surface_water", "Treated_wastewater", "Drinking_water"))
Plastic_usage <- factor (MAP_all_noNA$Usage_plastic_foil, levels = c("Yes_Bioplastics", "Yes_other_plastics", "No"))
Tillage <- factor (MAP_all_noNA$tillage, levels = c("Ploughing", "Non_inverted_tillage", "No"))
Crop_rotation <- factor(MAP_all_noNA$crop_rotation, levels = c("Yes", "No"))
pesticides <- factor(MAP_all_noNA$pesticides, levels = c("Yes", "No"))
```
...
data_Compost <- prepare_pie_data(MAP_all_noNA$Compost, "Compost")
data_Manure <- prepare_pie_data(MAP_all_noNA$Manure, "Manure")
data_Usage_plastic <- prepare_pie_data(MAP_all_noNA$Usage_plastic_foil, "Usage_plastic_foil")
data_Usage_plasticyesno <- prepare_pie_data(MAP_all_noNA$Plasticyesno, "Usage_plastic_foil_yesno")
data_tillage <- prepare_pie_data(MAP_all_noNA$tillage, "Tillage")
data_pesticides <- prepare_pie_data(MAP_all_noNA$pesticides, "pesticides")
data_croprotation <- prepare_pie_data(MAP_all_noNA$crop_rotation, "Crop_rotation")
data_irrigation <- prepare_pie_data(MAP_all_noNA$Irrigation, "Irrigation")
data_irrigation_yesno <- prepare_pie_data(MAP_all_noNA$Irrigationyesno, "Irrigation_yesno")
```
...
ggsave("Plots_all_litter/plot_pie.png", width = 10, height = 12, dpi = 600)
```
...
```{r}
model_NB1 <- glm.nb(
  formula = Total_plastics_count ~ (Compost + Manure + pesticides + Irrigationyesno + Plasticyesno + tillage)^2,
...
```{r}
model_NB <- glm.nb(
  formula = Total_plastics_count ~ (Compost + Manure + pesticides + Irrigationyesno + Plasticyesno)^2 + tillage + tillage:Manure + tillage:pesticides + tillage:Irrigationyesno + tillage:Plasticyesno,
  offset(log(Surface_m2)), 
  data = MAP_all_noNA, 
  control = glm.control(maxit = 100)
  )
...
fixed_y <- max(tillage_filtered$log_Total_plastic, na.rm = TRUE) * 1.1

#the plot
Boxplot_Compost_tillage <- ggplot(tillage_filtered, aes(x = Compost, y = log_Total_plastic, fill = tillage)) +
...
ggsave("Plots_all_litter/plot_barplot_fieldmanagement_withoutlog.png", width = 10, height = 12, dpi = 600)
```
...
```{r}
# Choose a fixed y-position slightly above the tallest value in the whole dataset
fixed_y_plastic <- max(MAP_all_noNA$log_Total_plastic, na.rm = TRUE) * 1.1
```
...
Boxplot_pest_plastic <- ggplot(MAP_all_noNA, aes(x = pesticides, y = log_Total_plastic, fill = Plasticyesno)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
...
# A2 / A17 / A21 /A5/ A6 - combi country
...
ggsave("Plots_all_litter/plot_barplot_A2.png", width = 15, height = 12, dpi = 600)
```
### A17
```{r}
...
ggsave("Plots_all_litter/plot_barplot_A17.png", width = 15, height = 12, dpi = 600)
```
### A21
```{r}
...
ggsave("Plots_all_litter/plot_barplot_A21.png", width = 15, height = 12, dpi = 600)
```
...
ggsave("Plots_all_litter/plot_barplot_A5A6.png", width = 15, height = 12, dpi = 600)
```


# A2 / A17 / A21 /A5 A6 - full dataset

```{r}
MAP_all_noNA[2:53] <- lapply(MAP_all_noNA[2:53], as.integer)
MAP_all_noNA[111] <- lapply(MAP_all_noNA[111], as.integer)
```
...
```
```{r}
em_A2 <- emmeans(model_NB_compost_A2, pairwise ~ Compost, adjust = "sidak")
em_A2

cld_em_A2 <- cld(em_A2, Letters = letters, adjust = "sidak")
print(cld_em_A2)

# Choose a fixed y-position slightly above the tallest value in the whole dataset
fixed_y <- max(MAP_all_noNA$log_A2, na.rm = TRUE) * 1.1

#the plot

Boxplot_A2_compost <- ggplot(MAP_all_noNA, aes(x = Compost, y = log_A2)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A2, aes(x = Compost, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A2 <- ggplot(summary_data_A2, aes(x = Compost, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A2, aes(x = Compost, y = 600, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean sheets (A2) Counts/Ha", x = "Compost Treatment") +
  theme_publish()
barplot_A2
```
...
```{r}
em_A17 <- emmeans(model_NB_compost_A17, pairwise ~ Compost, adjust = "sidak")
em_A17

cld_em_A17 <- cld(em_A17, Letters = letters, adjust = "sidak")
print(cld_em_A17)

# Choose a fixed y-position slightly above the tallest value in the whole dataset
fixed_y <- max(MAP_all_noNA$log_A17, na.rm = TRUE) * 1.1

#the plot

Boxplot_A17_compost <- ggplot(MAP_all_noNA, aes(x = Compost, y = log_A17)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A17, aes(x = Compost, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A17 <- ggplot(summary_data_A17, aes(x = Compost, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A17, aes(x = Compost, y = 300, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean A17 Counts/Ha", x = "Compost Treatment") +
  theme_publish()
barplot_A17

#ggsave("barplot_A17.png", dpi = 600, height = 5, width = 7, bg = "transparent")
```
...
```{r}
em_A21<- emmeans(model_NB_compost_A21, pairwise ~ Compost, adjust = "sidak")
em_A21

cld_em_A21 <- cld(em_A21, Letters = letters, adjust = "sidak")
print(cld_em_A21)

# Choose a fixed y-position slightly above the tallest value in the whole dataset
fixed_y <- max(MAP_all_noNA$log_A21, na.rm = TRUE) * 1.1

#the plot

Boxplot_A21_compost <- ggplot(MAP_all_noNA, aes(x = Compost, y = log_A21)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A21, aes(x = Compost, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_poster() + 
...
barplot_A21 <- ggplot(summary_data_A21, aes(x = Compost, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.1) +
  geom_text(data = cld_em_A21, aes(x = Compost, y = 20, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean A21 Counts/Ha", x = "Compost Treatment") +
  theme_publish()
barplot_A21
#ggsave("barplot_A21.png", dpi = 600, height = 5, width = 7, bg = "transparent")
```
...
```{r}
em_A5A6 <- emmeans(model_NB_compost_A5A6, pairwise ~ Compost, adjust = "sidak")
em_A5A6

cld_em_A5A6 <- cld(em_A5A6, Letters = letters, adjust = "sidak")
print(cld_em_A5A6)

# Choose a fixed y-position slightly above the tallest value in the whole dataset
fixed_y <- max(MAP_all_noNA$log_A5A6, na.rm = TRUE) * 1.1

#the plot

Boxplot_A5A6_compost <- ggplot(MAP_all_noNA, aes(x = Compost, y = log_A5A6)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A5A6, aes(x = Compost, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
```
```{r}
summary_data_A5A6 <- MAP_all_noNA %>%
  group_by(Compost) %>%
  summarise(
    mean_plastic = mean(A5_A6, na.rm = TRUE),
    se_plastic = sd(A5_A6, na.rm = TRUE) / sqrt(n())
...
barplot_A5A6 <- ggplot(summary_data_A5A6, aes(x = Compost, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A5A6, aes(x = Compost, y = 40, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean fibers (A5_A6) Counts/Ha", x = "Compost Treatment") +
  theme_publish()
barplot_A5A6
```
...
```
```{r}
em_A2_manure <- emmeans(model_NB_manure_A2, pairwise ~ Manure, adjust = "sidak")
em_A2_manure

cld_em_A2_manure <- cld(em_A2_manure, Letters = letters, adjust = "sidak")
print(cld_em_A2_manure)
...
Boxplot_A2_manure <- ggplot(MAP_all_noNA, aes(x = Manure, y = log_A2)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A2_manure, aes(x = Manure, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A2_manure <- ggplot(summary_data_A2, aes(x = Manure, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A2_manure, aes(x = Manure, y = 600, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean sheets (A2) Counts/Ha", x = "Manure Treatment") +
  theme_publish()
barplot_A2_manure
```
...
```{r}
em_A17_manure <- emmeans(model_NB_Manure_A17, pairwise ~ Manure, adjust = "sidak")
em_A17_manure

cld_em_A17_manure <- cld(em_A17_manure, Letters = letters, adjust = "sidak")
print(cld_em_A17_manure)
...
Boxplot_A17_manure <- ggplot(MAP_all_noNA, aes(x = Manure, y = log_A17)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A17_manure, aes(x = Manure, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A17_manure <- ggplot(summary_data_A17, aes(x = Manure, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A17_manure, aes(x = Manure, y = 300, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean hard plastcis (A17) Counts/Ha", x = "Manure Treatment") +
  theme_publish()
barplot_A17_manure
```
...
```{r}
em_A21_manure <- emmeans(model_NB_Manure_A21, pairwise ~ Manure, adjust = "sidak")
em_A21_manure

cld_em_A21_manure <- cld(em_A21_manure, Letters = letters, adjust = "sidak")
print(cld_em_A21_manure)
...
Boxplot_A21_Manure <- ggplot(MAP_all_noNA, aes(x = Manure, y = log_A21)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A21_manure, aes(x = Manure, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A21_manure <- ggplot(summary_data_A21, aes(x = Manure, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A21_manure, aes(x = Manure, y = 30, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean plastic stickers (A21) Counts/Ha", x = "Manure Treatment") +
  theme_publish()
barplot_A21_manure
```
...
```{r}
em_A5A6_Manure <- emmeans(model_NB_Manure_A5A6, pairwise ~ Manure, adjust = "sidak")
em_A5A6_Manure

cld_em_A5A6_Manure <- cld(em_A5A6_Manure, Letters = letters, adjust = "sidak")
print(cld_em_A5A6_Manure)
...
Boxplot_A5A6_Manure <- ggplot(MAP_all_noNA, aes(x = Manure, y = log_A5A6)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A5A6_Manure, aes(x = Manure, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A5A6_Manure <- ggplot(summary_data_A5A6, aes(x = Manure, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A5A6_Manure, aes(x = Manure, y = 75, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean fibers (A5_A6) Counts/Ha", x = "Manure Treatment") +
  theme_publish()
barplot_A5A6_Manure
```
...
```
```{r}
em_A2_pest <- emmeans(model_NB_pest_A2, pairwise ~ pesticides, adjust = "sidak")
em_A2_pest

cld_em_A2_pest <- cld(em_A2_pest, Letters = letters, adjust = "sidak")
print(cld_em_A2_pest)
...
Boxplot_A2_pest <- ggplot(MAP_all_noNA, aes(x = pesticides, y = log_A2)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A2_pest, aes(x = pesticides, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A2_pest <- ggplot(summary_data_A2, aes(x = pesticides, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A2_pest, aes(x = pesticides, y = 600, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean sheets (A2) Counts/Ha", x = "Pesticides Treatment") +
  theme_publish()
barplot_A2_pest
```
...
```{r}
em_A17_pest <- emmeans(model_NB_pest_A17, pairwise ~ pesticides, adjust = "sidak")
em_A17_pest

cld_em_A17_pest <- cld(em_A17_pest, Letters = letters, adjust = "sidak")
print(cld_em_A17_pest)
...
Boxplot_A17_pest <- ggplot(MAP_all_noNA, aes(x = pesticides, y = log_A17)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A17_pest, aes(x = pesticides, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A17_pest <- ggplot(summary_data_A17, aes(x = pesticides, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A17_pest, aes(x = pesticides, y = 300, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean hard plsatics (A17) Counts/Ha", x = "Pesticides Treatment") +
  theme_publish()
barplot_A17_pest
```
...
```{r}
em_A21_pest <- emmeans(model_NB_pest_A21, pairwise ~ pesticides, adjust = "sidak")
em_A21_pest

cld_em_A21_pest <- cld(em_A21_pest, Letters = letters, adjust = "sidak")
print(cld_em_A21_pest)
...
Boxplot_A21_pest <- ggplot(MAP_all_noNA, aes(x = pesticides, y = log_A21)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A21_pest, aes(x = pesticides, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A21_pest <- ggplot(summary_data_A21, aes(x = pesticides, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A21_pest, aes(x = pesticides, y = 30, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean plastic stickers (A21) Counts/Ha", x = "Pesticides Treatment") +
  theme_publish()
barplot_A21_pest
```
...
```{r}
em_A5A6_pest <- emmeans(model_NB_pest_A5A6, pairwise ~ pesticides, adjust = "sidak")
em_A5A6_pest

cld_em_A5A6_pest <- cld(em_A5A6_pest, Letters = letters, adjust = "sidak")
print(cld_em_A5A6_pest)

# Choose a fixed y-position slightly above the tallest value in the whole dataset
fixed_y <- max(MAP_all_noNA$log_A5A6, na.rm = TRUE) * 1.1
#the plot

Boxplot_A5A6_pest <- ggplot(MAP_all_noNA, aes(x = pesticides, y = log_A5A6)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A5A6_pest, aes(x = pesticides, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A5A6_pest <- ggplot(summary_data_A5A6, aes(x = pesticides, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A5A6_pest, aes(x = pesticides, y = 75, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean fibers (A5_A6) Counts/Ha", x = "Pesticides Treatment") +
  theme_publish()
barplot_A5A6_pest
```
...
```
```{r}
em_A2_irri <- emmeans(model_NB_irri_A2, pairwise ~ Irrigationyesno, adjust = "sidak")
em_A2_irri

cld_em_A2_irri <- cld(em_A2_irri, Letters = letters, adjust = "sidak")
print(cld_em_A2_irri)
...
Boxplot_A2_irri <- ggplot(MAP_all_noNA, aes(x = Irrigationyesno, y = log_A2)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A2_irri, aes(x = Irrigationyesno, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
```{r}
summary_data_A2 <- MAP_all_noNA %>%
  group_by(Irrigationyesno) %>%
  summarise(
    mean_plastic = mean(A2, na.rm = TRUE),
    se_plastic = sd(A2, na.rm = TRUE) / sqrt(n())
  )

barplot_A2_irri <- ggplot(summary_data_A2, aes(x = Irrigationyesno, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A2_irri, aes(x = Irrigationyesno, y = 800, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean sheets (A2) Counts/Ha", x = "Irrigation Treatment") +
  theme_publish()
barplot_A2_irri
```
...
```{r}
em_A17_irri <- emmeans(model_NB_irri_A17, pairwise ~ Irrigationyesno, adjust = "sidak")
em_A17_irri

cld_em_A17_irri <- cld(em_A17_irri, Letters = letters, adjust = "sidak")
print(cld_em_A17_irri)
...
Boxplot_A17_irri <- ggplot(MAP_all_noNA, aes(x = Irrigationyesno, y = log_A17)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A17_irri, aes(x = Irrigationyesno, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A17_irri <- ggplot(summary_data_A17, aes(x = Irrigationyesno, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A17_irri, aes(x = Irrigationyesno, y = 300, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean hard plastics (A17) Counts/Ha", x = "Irrigation Treatment") +
  theme_publish()
barplot_A17_irri
```
...
```{r}
em_A21_irri <- emmeans(model_NB_irri_A21, pairwise ~ Irrigationyesno, adjust = "sidak")
em_A21_irri

em_A21_irri <- cld(em_A21_irri, Letters = letters, adjust = "sidak")
print(em_A21_irri)
...
Boxplot_A21_irri <- ggplot(MAP_all_noNA, aes(x = Irrigationyesno, y = log_A21)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = em_A21_irri, aes(x = Irrigationyesno, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A21_irri <- ggplot(summary_data_A21, aes(x = Irrigationyesno, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = em_A21_irri, aes(x = Irrigationyesno, y = 30, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean plastic stickers (A21) Counts/Ha", x = "Irrigation Treatment") +
  theme_publish()
barplot_A21_irri
```
...
```{r}
em_A5A6_irri <- emmeans(model_NB_irri_A5A6, pairwise ~ Irrigationyesno, adjust = "sidak")
em_A5A6_irri

cld_em_A5A6_irri <- cld(em_A5A6_irri, Letters = letters, adjust = "sidak")
print(cld_em_A5A6_irri)
...
Boxplot_A5A6_irri <- ggplot(MAP_all_noNA, aes(x = Irrigationyesno, y = log_A5A6)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A5A6_irri, aes(x = Irrigationyesno, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A5A6_irri <- ggplot(summary_data_A5A6, aes(x = Irrigationyesno, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A5A6_irri, aes(x = Irrigationyesno, y = 40, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean fibers (A5_A6) Counts/Ha", x = "Irrigation Treatment") +
  theme_publish()
barplot_A5A6_irri
```
...
```{r}
em_A2_plastic <- emmeans(model_NB_plastic_A2, pairwise ~ Plasticyesno, adjust = "sidak")
em_A2_plastic

cld_em_A2_plastic <- cld(em_A2_plastic, Letters = letters, adjust = "sidak")
print(cld_em_A2_plastic)
...
Boxplot_A2_plastic <- ggplot(MAP_all_noNA, aes(x = Plasticyesno, y = log_A2)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A2_plastic, aes(x = Plasticyesno, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
```{r}
summary_data_A2 <- MAP_all_noNA %>%
  group_by(Plasticyesno) %>%
  summarise(
    mean_plastic = mean(A2, na.rm = TRUE),
    se_plastic = sd(A2, na.rm = TRUE) / sqrt(n())
  )

barplot_A2_plastic <- ggplot(summary_data_A2, aes(x = Plasticyesno, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A2_plastic, aes(x = Plasticyesno, y = 2000, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean A2 Counts/Ha", x = "Plastic usage") +
  theme_publish()
barplot_A2_plastic
#ggsave("barplot_A2_plastic.png", dpi = 600, height = 5, width = 7, bg = "transparent")
```


```{r}
summary <- MAP_all_noNA %>%
  group_by(Plasticyesno) %>% #onderverdelen in groepen, je kan hier ook meerder aan meegeven zoals bv compost
  dplyr::summarize(Mean = mean(Total_plastics, na.rm = TRUE),
             n = n(),
             SD = sd(Total_plastics,na.rm = TRUE),
             SE = sd(Total_plastics, na.rm = TRUE)/sqrt(n),
...
```{r}
em_A17_plastic <- emmeans(model_NB_plastic_A17, pairwise ~ Plasticyesno, adjust = "sidak")
em_A17_plastic

cld_em_A17_plastic <- cld(em_A17_plastic, Letters = letters, adjust = "sidak")
print(cld_em_A17_plastic)
...
Boxplot_A17_plastic <- ggplot(MAP_all_noNA, aes(x = Plasticyesno, y = log_A17)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A17_plastic, aes(x = Plasticyesno, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A17_plastic <- ggplot(summary_data_A17, aes(x = Plasticyesno, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A17_plastic, aes(x = Plasticyesno, y = 400, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean A17 Counts/Ha", x = "Plastic usage") +
  theme_publish()
barplot_A17_plastic

#ggsave("barplot_A17_plastic.png", dpi = 600, height = 5, width = 7, bg = "transparent")
```

### A21 - combi plastic
```{r}
model_NB_plastic_A21 <- glm.nb(
  formula = A21 ~ Plasticyesno, 
  offset(log(Surface_m2)), 
  data = MAP_all_noNA, 
  control = glm.control(maxit = 100))
...
```{r}
em_A21_plastic <- emmeans(model_NB_plastic_A21, pairwise ~ Plasticyesno, adjust = "sidak")
em_A21_plastic

cld_em_A21_plastic <- cld(em_A21_plastic, Letters = letters, adjust = "sidak")
print(cld_em_A21_plastic)
...
Boxplot_A21_plastic <- ggplot(MAP_all_noNA, aes(x = Plasticyesno, y = log_A21)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A21_plastic, aes(x = Plasticyesno, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A21_plastic <- ggplot(summary_data_A21, aes(x = Plasticyesno, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A21_plastic, aes(x = Plasticyesno, y = 30, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean plastic stickers (A21) Counts/Ha", x = "Plastic Treatment") +
  theme_publish()
barplot_A21_plastic
```
...
```{r}
em_A5A6_plastic <- emmeans(model_NB_plastic_A5A6, pairwise ~ Plasticyesno, adjust = "sidak")
em_A5A6_plastic

cld_em_A5A6_plastic <- cld(em_A5A6_plastic, Letters = letters, adjust = "sidak")
print(cld_em_A5A6_plastic)
...
Boxplot_A5A6_plastic <- ggplot(MAP_all_noNA, aes(x = Plasticyesno, y = log_A5A6)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
  #significant letter
  geom_text(data = cld_em_A5A6_plastic, aes(x = Plasticyesno, y = fixed_y, label = .group), inherit.aes = FALSE, fontface = "bold") +
  theme_publish() + 
...
barplot_A5A6_plastic <- ggplot(summary_data_A5A6, aes(x = Plasticyesno, y = mean_plastic)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = mean_plastic - se_plastic, ymax = mean_plastic + se_plastic),
                width = 0.2) +
  geom_text(data = cld_em_A5A6_plastic, aes(x = Plasticyesno, y = 100, label = .group), inherit.aes = FALSE, fontface = "bold") +
  labs(y = "Mean fibers (A5_A6) Counts/Ha", x = "Plastic Treatment") +
  theme_publish()
barplot_A5A6_plastic
```
...
```{r}
plot_barplot_A2 <- ((barplot_A2 + barplot_A2_manure + barplot_A2_plastic) / (barplot_A2_pest + barplot_A2_irri) ) + plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = "A")
 
ggsave("Plots_all_litter/plot_barplot_A2_fulldataset.png", width = 15, height = 12, dpi = 600)
```

### A17

```{r}
plot_barplot_A17 <- ((barplot_A17 + barplot_A17_manure + barplot_A17_plastic) / (barplot_A17_pest + barplot_A17_irri) ) + plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = "A")
 
ggsave("Plots_all_litter/plot_barplot_A17_fulldataset.png", width = 15, height = 12, dpi = 600)
```

### A21

```{r}
plot_barplot_A21 <- ((barplot_A21 + barplot_A21_manure + barplot_A21_plastic) / (barplot_A21_pest + barplot_A21_irri) ) + plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = "A")
 
ggsave("Plots_all_litter/plot_barplot_A21_fulldataset.png", width = 15, height = 12, dpi = 600)
```

### A5A6

```{r}
plot_barplot_A5A6 <- ((barplot_A5A6 + barplot_A5A6_Manure + barplot_A5A6_plastic) / (barplot_A5A6_pest + barplot_A5A6_irri) ) + plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = "A")
 
ggsave("Plots_all_litter/plot_barplot_A5A6_fulldataset.png", width = 15, height = 12, dpi = 600)
```

###A2-A17 - plastic

```{r}
plot_barplot_A2A17 <- ((barplot_A2_plastic + barplot_A17_plastic) + plot_layout(guides = 'collect')) +
  plot_annotation(tag_levels = "A", theme = theme(plot.background = element_blank()))
 
ggsave("plot_barplot_A2A17.png", width = 7, height = 5, dpi = 600, bg = "transparent")
```

###A2-A17 - compost

```{r}
plot_barplot_A2A17_compost <- ((barplot_A17 + barplot_A21) + plot_layout(guides = 'collect')) +
  plot_annotation(tag_levels = "A", theme = theme(plot.background = element_blank()))

ggsave("plot_barplot_A2A17_compost.png", width = 8, height = 5, dpi = 600, bg = "transparent")
```
...
model_NB_distance <- glm.nb(
  formula = Total_plastics_count ~ distance_city + Distance_primroutes + distance_highway + min_distance_IndorW + distance_village, 
  offset(log(Surface_m2)), 
...
model with the minimum distance to cities an villages taken into 1 parameter
```{r}
model_NB_distance_1 <- glm.nb(
  formula = Total_plastics_count ~ min_city_village + Distance_primroutes + distance_highway + min_distance_IndorW, 
  offset(log(Surface_m2)), 
  data = MAP_Belgium_noNA, 
  control = glm.control(maxit = 100))
...
```{r}
# Compute correlation
cor_value <- cor(MAP_Belgium_noNA$log_Total_plastic, MAP_Belgium_noNA$distance_city)
pred <- predict(model_NB_distance, type = "response")
# Plot
cor_plot_distance_city <- ggplot(MAP_Belgium_noNA, aes(x = distance_city, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
...
```{r}
# Compute correlation
cor_value <- cor(MAP_Belgium_noNA$log_Total_plastic, MAP_Belgium_noNA$Distance_primroutes)
pred <- predict(model_NB_distance, type = "response")
# Plot
cor_plot_distance_primroutes <- ggplot(MAP_Belgium_noNA, aes(x = Distance_primroutes, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
    method.args = list(family = negative.binomial(theta = model_NB_distance$theta)),
...
```{r}
# Compute correlation
cor_value <- cor(MAP_Belgium_noNA$log_Total_plastic, MAP_Belgium_noNA$distance_highway)
pred <- predict(model_NB_distance, type = "response")
# Plot
cor_plot_distance_highway <- ggplot(MAP_Belgium_noNA, aes(x = distance_highway, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
    method.args = list(family = negative.binomial(theta = model_NB_distance$theta)),
...
```{r}
# Compute correlation
cor_value <- cor(MAP_Belgium_noNA$log_Total_plastic, MAP_Belgium_noNA$min_distance_IndorW)
pred <- predict(model_NB_distance, type = "response")
# Plot
cor_plot_distance_IndorW <- ggplot(MAP_Belgium_noNA, aes(x = min_distance_IndorW, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
    method.args = list(family = negative.binomial(theta = model_NB_distance$theta)),
...
```{r}
# Compute correlation
cor_value <- cor(MAP_Belgium_noNA$log_Total_plastic, MAP_Belgium_noNA$distance_village)
pred <- predict(model_NB_distance, type = "response")
# Plot
cor_plot_distance_Village <- ggplot(MAP_Belgium_noNA, aes(x = distance_village, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
    method.args = list(family = negative.binomial(theta = model_NB_distance$theta)),
...
```{r}
# Compute correlation
cor_value <- cor(MAP_Belgium_noNA$log_Total_plastic, MAP_Belgium_noNA$min_city_village)
pred <- predict(model_NB_distance_1, type = "response")
# Plot
cor_plot_distance_Village_city <- ggplot(MAP_Belgium_noNA, aes(x = min_city_village, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
...
## NB model
```{r}
model_NB_distance_Vl <- glm.nb(
  formula = Total_plastics_count ~ distance_city + Distance_primroutes + distance_highway + min_distance_IndorW + distance_village, 
  offset(log(Surface_m2)), 
  data = MAP_flanders, 
  control = glm.control(maxit = 100))
    
summary(model_NB_distance_Vl)
par(mfrow = c(2, 2))  # set up 4 plots on one screen
plot(model_NB_distance_Vl)
```

```{r}
model_NB_distance_Vl_1 <- glm.nb(
  formula = Total_plastics_count ~ min_city_village + Distance_primroutes + distance_highway + min_distance_IndorW, 
  offset(log(Surface_m2)), 
  data = MAP_flanders, 
  control = glm.control(maxit = 100))
    
summary(model_NB_distance_Vl_1)
par(mfrow = c(2, 2))  # set up 4 plots on one screen
plot(model_NB_distance_Vl_1)
```
## min distance village and city

```{r}
# Compute correlation
cor_value <- cor(MAP_flanders$log_Total_plastic, MAP_flanders$min_city_village)
pred <- predict(model_NB_distance_Vl_1, type = "response")
# Plot
cor_plot_distance_Village_city_vl <- ggplot(MAP_flanders, aes(x = min_city_village, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
    method.args = list(family = negative.binomial(theta = model_NB_distance_Vl_1$theta)),
    se = TRUE) +
  annotate(
    "text",
    x = Inf, y = Inf,
  ...
  theme_publish()
cor_plot_distance_Village_city_vl
```
...
```{r}
# Compute correlation
cor_value <- cor(MAP_flanders$log_Total_plastic, MAP_flanders$Distance_primroutes)
pred <- predict(model_NB_distance_Vl_1, type = "response")
# Plot
cor_plot_distance_primroutes_vl <- ggplot(MAP_flanders, aes(x = Distance_primroutes, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
    method.args = list(family = negative.binomial(theta = model_NB_distance_Vl_1$theta)),
  ...
  theme_publish()
cor_plot_distance_primroutes_vl
```
...
```{r}
model_NB_distance_W <- glm.nb(
  formula = Total_plastics_count ~ min_city_village + Distance_primroutes + distance_highway + min_distance_IndorW, 
  offset(log(Surface_m2)), 
  data = MAP_Wallonia, 
  control = glm.control(maxit = 100))
    
summary(model_NB_distance_W)
par(mfrow = c(2, 2))  # set up 4 plots on one screen
plot(model_NB_distance_W)
```
## min distance village and city

```{r}
# Compute correlation
cor_value <- cor(MAP_Wallonia$log_Total_plastic, MAP_Wallonia$min_city_village)
pred <- predict(model_NB_distance_W, type = "response")
# Plot
cor_plot_distance_Village_city_W <- ggplot(MAP_Wallonia, aes(x = min_city_village, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
    method.args = list(family = negative.binomial(theta = model_NB_distance_W$theta)),
    se = TRUE) +
  annotate(
    "text",
    x = Inf, y = Inf,
  ...
  theme_publish()
cor_plot_distance_Village_city_W
```
...
```{r}
# Compute correlation
cor_value <- cor(MAP_Wallonia$log_Total_plastic, MAP_Wallonia$Distance_primroutes)
pred <- predict(model_NB_distance_W, type = "response")
# Plot
cor_plot_distance_primroutes_W <- ggplot(MAP_Wallonia, aes(x = Distance_primroutes, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
    method.args = list(family = negative.binomial(theta = model_NB_distance_W$theta)),
  ...
  theme_publish()
cor_plot_distance_primroutes_W
```

## Corelation minimum distance industry and recycling
```{r}
# Compute correlation
cor_value <- cor(MAP_Wallonia$log_Total_plastic, MAP_Wallonia$min_distance_IndorW)
pred <- predict(model_NB_distance_W, type = "response")
# Plot
cor_plot_distance_IndorW_W <- ggplot(MAP_Wallonia, aes(x = min_distance_IndorW, y = log_Total_plastic)) +
  geom_point(size = 2) +
  geom_smooth(
    method = "glm",
    method.args = list(family = negative.binomial(theta = model_NB_distance_W$theta)),
  ...
  labs(
    x = "Minimum distance to industry or waste recycling (m)",
    y = "Log total plastic/Ha"
  ) +
  theme_publish()
cor_plot_distance_IndorW_W
```
# ggsave flanders and wallonia

```{r}
plot_flanders_wallonia <- ((cor_plot_distance_primroutes_vl + cor_plot_distance_primroutes_W)) + plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = "A")
 
ggsave("Plots_all_litter/plot_flanders_wallonia_primroutes.png", width = 10, height = 7, dpi = 600)
```

```{r}
plot_flanders_wallonia <- ((cor_plot_distance_Village_city_vl + cor_plot_distance_Village_city_W)) + plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = "A")
 
ggsave("Plots_all_litter/plot_flanders_wallonia_cityandV.png", width = 10, height = 7, dpi = 600)
```

# Bodemtextuur
```{r}
model_NB_bodemtextuur <- glm.nb(
  formula = Total_plastics ~ Soil_Texture, 
  offset(log(Surface_m2)), 
  data = MAP_all_noNA, 
  control = glm.control(maxit = 100)
)
...
# Choose a fixed y-position slightly above the tallest value in the whole dataset
fixed_y <- max(MAP_all_noNA$log_Total_plastic, na.rm = TRUE) * 1.1

#the plot

Boxplot_bodemtextuur <- ggplot(MAP_all_noNA, aes(x = Soil_Texture, y = log_Total_plastic)) +
  geom_boxplot(outlier.shape = 2, width = 0.7) +
  geom_jitter(width = 0.3, alpha = 0.5) +
  #mean
  stat_summary(fun = mean, geom = "crossbar", color = "lightgrey", width = 0.69) +
